      ...
      const updatedBorderCountries = await Promise.all(
        country.borderCountries.map(async (borderCountryCode) => {
          try {
            ...
            console.error(
              `Error fetching country ${borderCountryCode}:`,
              error
            );
            return "Unknown";
          }
    ...
    <div class="lg:w-1/2 flex flex-col gap-12">
      ...
      <div class="flex justify-between flex-col lg:flex-row gap-10 lg:gap-0">
        <div class="flex flex-col gap-2">
          <p>
            <strong class="dark:text-gray-200">Official Name:</strong>
            <span class="dark:text-gray-400">{country.officialName}</span>
          </p>
          <p>
          ...
          <p>
            <strong class="dark:text-gray-200">Region:</strong>
            <span class="dark:text-gray-400">{country.region}</span>
          </p>
          <p>
            <strong class="dark:text-gray-200">Sub-region:</strong>
            <span class="dark:text-gray-400">{country.subregion}</span>
          </p>
          <p>
            <strong class="dark:text-gray-200">Capital:</strong>
            <span class="dark:text-gray-400">{country.capital}</span>
          </p>
        </div>
        <div class="flex flex-col gap-2">
          <p>
            <strong class="dark:text-gray-200">Top Level Domain:</strong>
            <span class="dark:text-gray-400">{country.tld}</span>
          </p>
          <p>
            <strong class="dark:text-gray-200">Currencies:</strong>
            <span class="dark:text-gray-400">{country.currencies}</span>
          </p>
          <p>
            <strong class="dark:text-gray-200">Languages:</strong>
            <span class="dark:text-gray-400">{country.languages}</span>
          </p>
        </div>
      </div>
          ...
          <div class="flex flex-wrap gap-2">
            {#each country.borderCountries as borderCountry}
              <a
                ...
                class="block"
                ><p
                  class="dark:text-gray-400 dark:bg-[#2b3743] border shadow-5xl px-4 rounded-sm py-1 text-xs"
                >
                  {borderCountry}
                </p></a
              >
